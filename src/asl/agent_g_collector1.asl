...
+! get_locations : true <-	
	...
	.wait({+gold_locations(LocationList)});
	.print("recieved locations: " , LocationList);
	
	
	// kill agent if no gold is found
	if (LocationList == []){
		.kill_agent(agent_g_collector);
	}
...
+obstructed(X_travelled,Y_travelled,X_left, Y_left) <-
	...
	!attempt_to_move;
	!go_to_gold_after_obstuction;
	
	.
	
-! go_to_gold_after_obstuction : true <-
	.print("go_to_gold_after_obstuction failed");
	.
	
	
@go_to_gold_after_obstuction[atomic]
+! go_to_gold_after_obstuction : true <- 
	Hor = "horizontal";
	Ver = "vertical";
	?go_to(X,Y,Quantity);
 ...
 * for every gold location:
 * 		1 . try to move to the gold:
 * 			1.1 success: collect gold
 ...
 *		2.collect:
 * 			fail: go back to base and search for another gold location
 * 			success: stage 3.
 * 		3. return to base:
