...
+!move_around : true <- 
	...
	if (X < 70){
		...
		.print("sending d : " , D);
		.print("sending g : " , G);
		.send(agent_random_scan2, tell, recieved_resource_locations_d(D));
		.send(agent_random_scan2, tell, recieved_resource_locations_g(G));	
		.kill_agent(agent_random_scan);
		.print("sent");
	...
	if(not(B)){
		...
		scan(Scanrange);
		-+first_move(true);
		.print("making an initial move of left-down");
	...
	.print("current distance from base" , -Log_x, " " , -Log_y);
