...
//	rover.ia.log_movement(-Log_x, -Log_y);
	...
	scan(6);
	?obstacles(List);
	.print("Found obstacles in: " , List);
	.print("Nearest gap found in [0,1] from rover");
	move(0,1);
	move(3,0);
	move(0,-2);
	scan(6);
...
+ resource_found (ResourceType, Quantity, XDist, YDist) : true <-
	...
	// Adds the new diamond location to the list of locations
		...
		?obstacles(List);
		.concat(List, [[XDist-Xbase, YDist-Ybase]], List_prime_d);
		-+obstacles(List_prime_d);
	}
